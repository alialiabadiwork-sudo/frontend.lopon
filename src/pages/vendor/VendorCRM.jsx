...
export default function VendorCRM() {
  ...
  const handleAddNote = (e) => {
    ...
    // Update local selected state as well
    setSelectedCustomer({
      ...selectedCustomer,
      staffNotes: [newNoteText.trim(), ...selectedCustomer.staffNotes],
    });
    setNewNoteText('');
  };
  ...
  return (
    <div className="space-y-6">
      {/* 1. Page Header & Win-back Campaign Banner */}
      <div className="bg-white rounded-3xl p-5 sm:p-6 border border-slate-200/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="font-kal-1 text-xs text-slate-400 mt-0.5">
            تبدیل خریداران کوپن لوپُن به مشتریان دائمی سالن، ردیابی ارزش طول عمر (LTV) و یادداشت‌های پرسنلی
          </p>
        </div>
        ...
        <button
          ...
          className="h-11 px-5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-kal-3 font-bold text-xs rounded-xl flex items-center gap-2 shadow-sm transition-all cursor-pointer whitespace-nowrap"
        >
          <MessageSquare className="w-4 h-4" />
          <span>ارسال پیامک تخفیف اختصاصی سالن</span>
        </button>
      </div>
          ...
          >
            ...
            <div className="space-y-2 text-xs font-kal-2">
              ...
              {cust.staffNotes?.length > 0 && (
                <div className="bg-amber-50/70 text-amber-900 p-2.5 rounded-xl border border-amber-200/60 text-[11.5px] leading-relaxed line-clamp-2">
                  <strong>یادداشت پرسنلی:</strong> {cust.staffNotes[0]}
                </div>
              )}
            </div>
          </div>
      ...
      {/* 5. Customer Profile Drawer / Modal (پرونده اختصاصی) */}
      <AnimatePresence>
        {selectedCustomer && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" dir="rtl">
            ...
            >
              ...
              <div className="p-6 overflow-y-auto space-y-6">
                ...
                {/* Staff Notes Section (یادداشت‌های پرسنلی) */}
                <div className="space-y-3">
                  <h4 className="font-kal-3 font-bold text-slate-900 text-xs flex items-center gap-2">
                    ...
                    <span>یادداشت‌های پرسنلی (فرمول رنگ، حساسیت پوستی، ترجیحات مشتری)</span>
                  </h4>
                  ...
                  <form onSubmit={handleAddNote} className="flex gap-2">
                    <input
                      ...
                      placeholder="ثبت یادداشت جدید برای پرسنل..."
                      className="flex-1 h-10 px-4 text-xs font-kal-2 bg-slate-50 border border-slate-200 rounded-xl focus:border-purple-500 focus:bg-white outline-none"
                    />
                    <button
                      type="submit"
                      disabled={!newNoteText.trim()}
                      className="h-10 px-4 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white text-xs font-kal-3 font-bold rounded-xl transition-colors cursor-pointer"
                    >
                      افزودن یادداشت
                    </button>
                  </form>
                  ...
                  <div className="space-y-2">
                    {selectedCustomer.staffNotes?.map((note, idx) => (
                      <div
                        key={idx}
                        className="p-3 rounded-xl bg-amber-50/80 border border-amber-200 text-xs font-kal-2 text-amber-950 flex items-start gap-2"
                      >
                ...
                {/* Full History of Services (تاریخچه کامل خدمات) */}
                <div className="space-y-3">
                  <h4 className="font-kal-3 font-bold text-slate-900 text-xs flex items-center gap-2">
      ...
      {/* 6. Broadcast Marketing SMS Modal */}
      <AnimatePresence>
        {smsModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" dir="rtl">
            <motion.div
